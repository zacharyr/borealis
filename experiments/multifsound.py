...
class Multifsound(ExperimentPrototype):
    ...
    def __init__(self):
        ...
        if scf.opts.site_id in ["rkn"]:
            freqs = (10100, 10500, 10900, 11500, 12300, 13200) 
        ...
        slice_3['txfreq'] = freqs[2]
        slice_4['txfreq'] = freqs[3]
        slice_5['txfreq'] = freqs[4]
        slice_6['txfreq'] = freqs[5]

        list_of_slices = [slice_1, slice_2, slice_3, slice_4, slice_5, slice_6]
        sum_of_freq = 0
        for slice in list_of_slices:
            sum_of_freq += slice['txfreq']# kHz, oscillator mixer frequency on the USRP for TX
        rxctrfreq = txctrfreq = 11650


        super(Multifsound, self).__init__(cpid, txctrfreq=txctrfreq, rxctrfreq=rxctrfreq,
                comment_string='Multifsound inttime interfaced')
